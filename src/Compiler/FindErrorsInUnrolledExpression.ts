...
function FindErrorsInUnrolledExpression(expression: UnrolledExpression): ErrorObj[] {
    ...
    switch (expression.Type) {
        ...
        case "Return":
            return FindErrorsInUnrolledExpression(expression.Value)

        case "Unary": return FindErrorsInUnrolledExpression(expression.Operand)
        case "Binary": return FindErrorsInUnrolledExpression(expression.Left).concat(FindErrorsInUnrolledExpression(expression.Right))
        case "BinaryInconsistentPlurality": {
            const output: ErrorObj[] = []
            for (const dimension of expression.Left) for (const error of FindErrorsInUnrolledExpression(dimension)) output.push(error)
            for (const dimension of expression.Right) for (const error of FindErrorsInUnrolledExpression(dimension)) output.push(error)
            output.push({
        ...
        case "GetItem": {
            const output: ErrorObj[] = []
            for (const error of FindErrorsInUnrolledExpression(expression.Value)) output.push(error)
            for (let i = 0; i < expression.Of.length; i++) if (i != expression.Item) for (const error of FindErrorsInUnrolledExpression(expression.Of[i])) output.push(error)
            return output
        }

        case "GetItemOutOfRange": {
            const output: ErrorObj[] = []
            for (const dimension of expression.Of) for (const error of FindErrorsInUnrolledExpression(dimension)) output.push(error)
            output.push({
        ...
        case "Call": {
            ...
            for (const error of FindErrorsInUnrolledExpression(expression.Result)) output.push(error)
            return output
        }
        ...
        case "Let": {
            const output: ErrorObj[] = []
            for (const dimension of expression.Value) for (const error of FindErrorsInUnrolledExpression(dimension)) output.push(error)
            for (const error of FindErrorsInUnrolledExpression(expression.Then)) output.push(error)
            return output
        }
        ...
        case "LetWithoutIdentifier": return AssertErrorObj({
            ...
            StartIndex: expression.StartIndex,
            EndIndex: expression.EndIndex
        }).concat(FindErrorsInUnrolledExpression(expression.Then))
